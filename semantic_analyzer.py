...
class SemanticAnalyzer:
    ...
    def train_model(self):
        ...
        print(f"训练样本总数: {len(train_examples)}")
        print(f"A组正样本: {len(a_positive_pairs)}, B组正样本: {len(b_positive_pairs)}, 负样本: {len(negative_pairs)}")
        ...
        train_dataloader = DataLoader(
            train_examples,
            shuffle=True,
            batch_size=32,  # 增加batch_size以提高效率
        ...
        # 6. 创建评估器来监控loss
        # 创建一些验证样本用于评估
        eval_examples = train_examples[:200]  # 使用前400个样本作为验证集
        from sentence_transformers.evaluation import EmbeddingSimilarityEvaluator
        evaluator = EmbeddingSimilarityEvaluator.from_input_examples(
            eval_examples, 
            name='medical_similarity'
        )
        ...
        self.model.fit(
            ...
            evaluator=evaluator,  # 添加评估器
            epochs=2,
            show_progress_bar=True,
            optimizer_params={'lr': 2e-5},
            evaluation_steps=200,  # 每100步评估一次
            warmup_steps=200      # 前100步作为warmup
        )
